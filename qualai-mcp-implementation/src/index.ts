  ...
  try {
    switch (name) {
      ...
      case 'addDataSource': {
        ...
        // Get project to check methodology
        const project = db.getEntity(parsed.projectName);
        const methodology = project?.metadata?.methodology || 'general';

        // Auto-code the content
        const codingResult = await codingEngine.autoCoding({
          text: parsed.content,
          existingCodes: project?.metadata?.refinedCodebook?.map((c: any) => c.name) || [],
          methodology,
        });
        ...
        db.createEntity({
          ...
          observations: [
            ...
            `Auto-coded: ${codingResult.summary.totalCodes} codes`,
        ...
        return {
          content: [{
            type: 'text',
            text: `✅ Added ${parsed.sourceType} "${parsed.name}" to project "${parsed.projectName}"\n\nContent: ${parsed.content.length} characters\nAuto-coded: ${codingResult.summary.totalCodes} codes (${codingResult.summary.inVivoCodes} in-vivo, ${codingResult.summary.constructedCodes} constructed)\nSegments: ${codingResult.segments.length}\n\nYou can now run:\n- refineCodebook to consolidate codes\n- extractThemes to identify patterns`,
          }],
        };
      }
      ...
      case 'autoCoding': {
        ...
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({
              message: `Auto-coded ${parsed.text.length} characters`,
              result: {
      ...
      case 'refineCodebook': {
        ...
        // Load codes from project
        const project = db.getEntity(parsed.projectName);
        if (!project) {
          throw new Error(`Project "${parsed.projectName}" not found`);
        }

        // Get all codes from project data sources
        const relations = db.getRelations(parsed.projectName);
        const dataSources = relations
          .filter((r: any) => r.relationType === 'part_of')
          .map((r: any) => db.getEntity(r.from))
          .filter(Boolean);

        // Collect all codes from data sources (stored in metadata)
        const allCodes = [];
        for (const source of dataSources) {
          if (source?.metadata?.codes) {
            allCodes.push(...source.metadata.codes);
          }
        }
        ...
        if (allCodes.length === 0) {
          return {
            content: [{
              type: 'text',
              text: `⚠️ No codes found for project "${parsed.projectName}". Please run autoCoding first on your data sources.`,
            }],
          };
        }
        ...
        // Save refined codebook to project
        db.addObservation(parsed.projectName, `Refined codebook: ${allCodes.length} → ${result.refined.length} codes`);
        if (project.metadata) {
        ...
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({
              message: `Refined codebook for "${parsed.projectName}"`,
              summary: {
                original: allCodes.length,
                refined: result.refined.length,
                merged: result.merges.length,
              },
              refinedCodes: result.refined.map(c => ({
                name: c.name,
                definition: c.definition,
                frequency: c.frequency,
                type: c.type,
      ...
      case 'extractThemes': {
        ...
        // Load refined codebook from project
        const project = db.getEntity(parsed.projectName);
        if (!project) {
          throw new Error(`Project "${parsed.projectName}" not found`);
        }
        ...
        if (codes.length === 0) {
          return {
            content: [{
              type: 'text',
              text: `⚠️ No refined codebook found for "${parsed.projectName}". Please run refineCodebook first.`,
            }],
          };
        }
        ...
        // Save themes to project
        db.addObservation(parsed.projectName, `Extracted ${themes.length} themes (${parsed.mode} mode)`);
        if (project.metadata) {
        ...
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({
              message: `Extracted ${themes.length} themes using ${parsed.mode} approach`,
              themes: themes.map(t => ({
                name: t.name,
                description: t.description,
                prevalence: `${(t.prevalence * 100).toFixed(1)}%`,
                supportingCodes: t.supportingCodes.length,
                subThemes: t.subThemes?.length || 0,
              })),
              fullThemes: themes,
            }, null, 2),
          }],
      ...
      case 'buildGroundedTheory': {
        ...
        // Load codes and themes from project
        const project = db.getEntity(parsed.projectName);
        if (!project) {
          throw new Error(`Project "${parsed.projectName}" not found`);
        }
        ...
        if (codes.length === 0 || themes.length === 0) {
          return {
            content: [{
              type: 'text',
              text: `⚠️ Need both codes and themes to build theory.\n\nFound: ${codes.length} codes, ${themes.length} themes\n\nPlease run:\n1. autoCoding on your data\n2. refineCodebook\n3. extractThemes\n\nThen try buildGroundedTheory again.`,
            }],
          };
        }
        ...
        db.addObservation(parsed.projectName, `Built grounded theory: ${theory.title}`);
        if (project.metadata) {
        ...
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({
              message: `Built grounded theory for "${parsed.projectName}"`,
              theory: {
                title: theory.title,
                coreCategory: theory.coreCategory,
                paradigm: theory.paradigm,
                paradigmModel: theory.paradigmModel,
                storyline: theory.storyline,
                propositions: theory.theoreticalPropositions,
                relationships: theory.categoryRelationships.length,
              },
              fullTheory: theory,
            }, null, 2),
          }],
      ...
      case 'detectSaturation': {
        ...
        for (const source of dataSources) {
          if (source?.metadata?.codes) {
            codesBySource.set(source.name, source.metadata.codes);
          }
        }

        if (codesBySource.size === 0) {
          return {
            content: [{
              type: 'text',
              text: `⚠️ No coded data sources found for "${parsed.projectName}". Please code your data first.`,
            }],
          };
        }
        ...
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({
              message: `Saturation analysis for "${parsed.projectName}"`,
              level: parsed.level,
              saturated: saturation.saturated,
              saturationRate: `${(saturation.saturationRate * 100).toFixed(1)}%`,
              newCodesPerSource: saturation.newCodesPerSource,
              recommendation: saturation.recommendation,
              dataSources: codesBySource.size,
            }, null, 2),
          }],
        };
      }
      ...
      case 'generateConceptMap': {
        const parsed = generateConceptMapSchema.parse(args);
        ...
        if (codes.length === 0 || themes.length === 0) {
          return {
            content: [{
              type: 'text',
              text: `⚠️ Need codes and themes to generate concept map. Please run analysis first.`,
            }],
          };
        }
        ...
        return {
          content: [{
            type: 'text',
            text: JSON.stringify({
              message: `Generated ${parsed.style || 'network'} concept map`,
              nodes: conceptMap.nodes.length,
              edges: conceptMap.edges.length,
              conceptMap,
            }, null, 2),
          }],
        };
